...
import AccountView from '@/components/status/views/AccountView';
import type { AccountPageProps } from '@/types/status';

export default function AccountPage({ meta, frame, user, flash, error }: AccountPageProps) {
    return (
        <>
            ...
            <DashboardShell
                ...
                authControls={<AuthControls authName={user.name} />}
                authName={user.name}
            >
                <AccountView user={user} flash={flash} error={error} />
            </DashboardShell>
        </>
    );
}
